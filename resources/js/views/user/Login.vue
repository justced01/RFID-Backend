            ...
            <div class="basis-3/4">
                <form @submit.prevent="handleLogin" class="flex flex-col w-[364px] ">
                    <div class="flex flex-col flex-start relative">
                        ...
                        <input type="text" name="email" placeholder="Email address" v-model="email" class="my-3 rounded-lg py-[16px] px-[8px] pl-10 border border-[#464646] placeholder:text-[13px] placeholder:font-medium">
                    </div>
                    <div class="flex flex-col flex-start relative">
                        ...
                        <input type="password" name="password" placeholder="Password" v-if="hidePassword" v-model="password" class="my-3 rounded-lg py-[16px] px-[8px] pl-10 border border-[#464646] placeholder:text-[13px] placeholder:font-medium">
                        <input type="text" name="password" placeholder="Password" v-else v-model="password" class="my-3 rounded-lg py-[16px] px-[8px] pl-10 border border-[#464646] placeholder:text-[13px] placeholder:font-medium">
                    </div>
            ...
            if(this.errorMessages.length === 0){
                ...
                .then(response => {
                    if(response.data.status === 200){
                        this.emitter.emit('access_token', response.data.access_token)
